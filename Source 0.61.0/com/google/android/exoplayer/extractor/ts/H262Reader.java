...
final class H262Reader extends ElementaryStreamReader {
    ...
    public H262Reader(TrackOutput output) {
        ...
        this.csdBuffer = new CsdBuffer(AccessibilityNodeInfoCompat.ACTION_CLEAR_ACCESSIBILITY_FOCUS);
    }
    ...
    public void consume(ParsableByteArray data) {
        if (data.bytesLeft() > 0) {
            ...
            while (true) {
                ...
                int startCodeValue = data.data[startCodeOffset + 3] & NalUnitUtil.EXTENDED_SAR;
    ...
    private static Pair<MediaFormat, Long> parseCsdBuffer(CsdBuffer csdBuffer) {
        ...
        int secondByte = csdData[5] & NalUnitUtil.EXTENDED_SAR;
        int width = ((csdData[4] & NalUnitUtil.EXTENDED_SAR) << 4) | (secondByte >> 4);
        int i = (secondByte & 15) << 8;
        int height = i | (csdData[6] & NalUnitUtil.EXTENDED_SAR);
