...
public final class ScalarSynchronousObservable<T> extends Observable<T> {
    ...
    class C15601 implements OnSubscribe<T> {
    ...
    class C15612 implements Func1<Action0, Subscription> {
        final /* synthetic */ EventLoopsScheduler val$els;

        C15612(EventLoopsScheduler eventLoopsScheduler) {
    ...
    class C15644 implements OnSubscribe<R> {
        ...
        public void call(Subscriber<? super R> child) {
            Observable<? extends R> o = (Observable) this.val$func.call(ScalarSynchronousObservable.this.f763t);
            if (o instanceof ScalarSynchronousObservable) {
                child.setProducer(ScalarSynchronousObservable.createProducer(child, ((ScalarSynchronousObservable) o).f763t));
            } else {
    ...
    public Observable<T> scalarScheduleOn(Scheduler scheduler) {
        ...
        if (scheduler instanceof EventLoopsScheduler) {
            onSchedule = new C15612((EventLoopsScheduler) scheduler);
        } else {
            onSchedule = new C15633(scheduler);
        }
        return Observable.create(new ScalarAsyncOnSubscribe(this.f763t, onSchedule));
    }

    public <R> Observable<R> scalarFlatMap(Func1<? super T, ? extends Observable<? extends R>> func) {
        return Observable.create(new C15644(func));
    }
}
