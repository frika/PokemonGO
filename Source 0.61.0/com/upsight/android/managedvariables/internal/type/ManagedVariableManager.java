...
public class ManagedVariableManager {
    ...
    static class C11361 extends HashMap<Class<? extends ManagedVariable>, Class<? extends ManagedVariableModel>> {
        C11361() {
    ...
    class C11372 implements Action1<T> {
        final /* synthetic */ Listener val$listener;

        C11372(Listener listener) {
    ...
    class C11383 implements Action1<ManagedVariableModel> {
        final /* synthetic */ Class val$clazz;
        final /* synthetic */ Listener val$listener;
        final /* synthetic */ String val$tag;

        C11383(String str, Listener listener, Class cls) {
            this.val$tag = str;
            this.val$listener = listener;
            this.val$clazz = cls;
        }
    ...
    class C11394 implements Action1<Throwable> {
        final /* synthetic */ Listener val$listener;

        C11394(Listener listener) {
            this.val$listener = listener;
        }
    ...
    class C11405 implements Func1<ManagedVariableModel, Boolean> {
        final /* synthetic */ Class val$clazz;
        final /* synthetic */ String val$tag;

        C11405(Class cls, String str) {
            this.val$clazz = cls;
            this.val$tag = str;
        }
    ...
    public <T extends ManagedVariable> UpsightSubscription fetch(Class<T> clazz, String tag, Listener<T> listener) {
        ...
        synchronized (this.mCache) {
            ...
            if (cachedVariable != null) {
                subscription = Subscriptions.from(Observable.just(cachedVariable).observeOn(this.mCallbackScheduler).subscribe(new C11372(listener)));
            } else {
                subscription = Subscriptions.from(fetchDataStoreObservable(clazz, tag).subscribe(new C11383(tag, listener, clazz), new C11394(listener)));
            }
        }
        return subscription;
    }
    ...
    private <T extends ManagedVariable> Observable<? extends ManagedVariableModel> fetchDataStoreObservable(Class<T> clazz, String tag) {
        return this.mDataStore.fetchObservable((Class) sModelMap.get(clazz)).filter(new C11405(clazz, tag)).defaultIfEmpty(null);
    }
