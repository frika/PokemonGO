...
class Analytics implements UpsightAnalyticsApi {
    ...
    public void recordSessionless(UpsightAnalyticsEvent event) {
        record(event, this.mSessionManager.getBackgroundSession());
    }

    public void record(UpsightAnalyticsEvent event) {
        record(event, this.mSessionManager.getSession());
    }
    ...
    private void record(UpsightAnalyticsEvent event, Session session) {
        if (Upsight.isEnabled(this.mUpsight)) {
            ...
            Integer sessionNum = session.getSessionNumber();
            Long prevTos = session.getPreviousTos();
        ...
        this.mUpsight.getLogger().m574d(Upsight.LOG_TAG, "Recording of " + event.getType() + " event failed because SDK is disabled", new Object[0]);
    }
