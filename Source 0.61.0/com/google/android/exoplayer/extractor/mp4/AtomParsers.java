...
final class AtomParsers {
    ...
    public static Track parseTrak(ContainerAtom trak, LeafAtom mvhd, boolean isQuickTime) {
        ...
        if (duration == -1) {
            ...
            durationUs = Util.scaleLargeTimestamp(duration, C0277C.MICROS_PER_SECOND, movieTimescale);
        }
    ...
    public static TrackSampleTable parseStbl(Track track, ContainerAtom stblAtom) {
        ...
        if (track.editListDurations == null) {
            Util.scaleLargeTimestampsInPlace(timestamps, C0277C.MICROS_PER_SECOND, track.timescale);
            return new TrackSampleTable(offsets, sizes, maximumSize, timestamps, flags);
        } else if (track.editListDurations.length == 1 && track.editListDurations[0] == 0) {
            for (i = 0; i < timestamps.length; i++) {
                timestamps[i] = Util.scaleLargeTimestamp(timestamps[i] - track.editListMediaTimes[0], C0277C.MICROS_PER_SECOND, track.timescale);
            }
            return new TrackSampleTable(offsets, sizes, maximumSize, timestamps, flags);
        } else {
            long mediaTime;
            ...
            for (i = 0; i < track.editListDurations.length; i++) {
                ...
                if (mediaTime != -1) {
                    ...
                    for (int j = startIndex; j < endIndex; j++) {
                        editedTimestamps[sampleIndex] = Util.scaleLargeTimestamp(pts, C0277C.MICROS_PER_SECOND, track.movieTimescale) + Util.scaleLargeTimestamp(timestamps[j] - mediaTime, C0277C.MICROS_PER_SECOND, track.timescale);
    ...
    private static TkhdData parseTkhd(ParsableByteArray tkhd) {
        ...
        if (a00 == 0 && a01 == AccessibilityNodeInfoCompat.ACTION_CUT) {
            i = -65536;
            if (a10 == r0 && a11 == 0) {
                rotationDegrees = 90;
        ...
        if (a00 == 0) {
            i = -65536;
            if (a01 == r0 && a10 == AccessibilityNodeInfoCompat.ACTION_CUT && a11 == 0) {
                rotationDegrees = 270;
                return new TkhdData(trackId, duration, rotationDegrees);
            }
        }
