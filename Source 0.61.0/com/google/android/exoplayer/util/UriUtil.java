...
public final class UriUtil {
    ...
    public static String resolve(String baseUri, String referenceUri) {
        ...
        if (refIndices[PATH] != 0) {
        ...
        } else if (refIndices[PATH] != refIndices[QUERY] && referenceUri.charAt(refIndices[PATH]) == IOUtils.DIR_SEPARATOR_UNIX) {
            ...
            uri.append(baseUri, 0, baseIndices[PATH]).append(IOUtils.DIR_SEPARATOR_UNIX).append(referenceUri);
    ...
    private static String removeDotSegments(StringBuilder uri, int offset, int limit) {
        ...
        while (i <= limit) {
            ...
            if (i == segmentStart + PATH && uri.charAt(segmentStart) == FilenameUtils.EXTENSION_SEPARATOR) {
                uri.delete(segmentStart, nextSegmentStart);
                limit -= nextSegmentStart - segmentStart;
                i = segmentStart;
            } else if (i == segmentStart + QUERY && uri.charAt(segmentStart) == FilenameUtils.EXTENSION_SEPARATOR && uri.charAt(segmentStart + PATH) == FilenameUtils.EXTENSION_SEPARATOR) {
                int removeFrom;
    ...
    private static int[] getUriIndices(String uriString) {
        ...
        if (TextUtils.isEmpty(uriString)) {
            ...
            if (schemeIndex + QUERY < queryIndex && uriString.charAt(schemeIndex + PATH) == IOUtils.DIR_SEPARATOR_UNIX && uriString.charAt(schemeIndex + QUERY) == IOUtils.DIR_SEPARATOR_UNIX) {
                hasAuthority = true;
            } else {
                hasAuthority = false;
            }
