...
public class PagerTitleStrip extends ViewGroup implements Decor {
    ...
    public void setNonPrimaryAlpha(@FloatRange(from = 0.0d, to = 1.0d) float alpha) {
        this.mNonPrimaryAlpha = ((int) (255.0f * alpha)) & NalUnitUtil.EXTENDED_SAR;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        childHeight -= padding;
        int childWidthSpec = MeasureSpec.makeMeasureSpec(Math.max(0, (int) (((float) widthSize) * DefaultLoadControl.DEFAULT_HIGH_BUFFER_LOAD)), Cue.TYPE_UNSET);
        int childHeightSpec = MeasureSpec.makeMeasureSpec(Math.min(0, childHeight), Cue.TYPE_UNSET);
        this.mPrevText.measure(childWidthSpec, childHeightSpec);
        this.mCurrText.measure(childWidthSpec, childHeightSpec);
        this.mNextText.measure(childWidthSpec, childHeightSpec);
