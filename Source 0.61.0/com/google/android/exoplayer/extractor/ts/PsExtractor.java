...
public final class PsExtractor implements Extractor {
    ...
    public boolean sniff(ExtractorInput input) throws IOException, InterruptedException {
        ...
        if (PACK_START_CODE != (((((scratch[0] & NalUnitUtil.EXTENDED_SAR) << 24) | ((scratch[PACKET_START_CODE_PREFIX] & NalUnitUtil.EXTENDED_SAR) << 16)) | ((scratch[2] & NalUnitUtil.EXTENDED_SAR) << 8)) | (scratch[3] & NalUnitUtil.EXTENDED_SAR)) || (scratch[4] & 196) != 68 || (scratch[6] & 4) != 4 || (scratch[8] & 4) != 4 || (scratch[9] & PACKET_START_CODE_PREFIX) != PACKET_START_CODE_PREFIX || (scratch[12] & 3) != 3) {
            return false;
        }
        input.advancePeekPosition(scratch[13] & 7);
        input.peekFully(scratch, 0, 3);
        if (PACKET_START_CODE_PREFIX != ((((scratch[0] & NalUnitUtil.EXTENDED_SAR) << 16) | ((scratch[PACKET_START_CODE_PREFIX] & NalUnitUtil.EXTENDED_SAR) << 8)) | (scratch[2] & NalUnitUtil.EXTENDED_SAR))) {
            z = false;
        }
        return z;
    }
    ...
    public int read(ExtractorInput input, PositionHolder seekPosition) throws IOException, InterruptedException {
        ...
        if (nextStartCode == PACK_START_CODE) {
            ...
            int streamId = nextStartCode & NalUnitUtil.EXTENDED_SAR;
