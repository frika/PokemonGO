...
public class BatchSender {
    ...
    class C10801 implements Action0 {
        final /* synthetic */ Request val$request;

        C10801(Request request) {
    ...
    class C10812 implements Action0 {
        final /* synthetic */ Request val$request;

        C10812(Request request) {
            this.val$request = request;
        }
    ...
    private class BatchSendTask implements Runnable {
        ...
        public void run() {
            if (!Upsight.isEnabled(BatchSender.this.mUpsight)) {
                BatchSender.this.sendFailed(this.mRequest, FailReason.SDK_DISABLED, SDK_DISABLED_ERROR);
            } else if (NetworkHelper.isConnected(BatchSender.this.mUpsight)) {
                try {
                    ...
                    BatchSender.this.mLogger.m576e("BatchSender", "Received " + resp.statusCode + " HTTP response code from server", new Object[0]);
    ...
    public void submitRequest(Request request) {
        this.mBatchSendExecutor.createWorker().schedule(new C10801(request));
    }
    ...
    private void sendFailed(Request request, FailReason failReason, String errorString) {
        ...
        if (tryCount.intValue() > 0) {
            ...
            this.mRetryExecutor.createWorker().schedule(new C10812(request), (long) this.mConfig.retryInterval, TimeUnit.SECONDS);
            return;
        }
        this.mTryCounts.remove(request);
        for (Packet packet : request.batch.getPackets()) {
