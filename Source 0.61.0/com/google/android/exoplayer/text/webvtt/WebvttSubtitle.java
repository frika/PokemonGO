        ...
        while (i < this.numCues) {
            if (this.cueTimesUs[i * 2] <= timeUs && timeUs < this.cueTimesUs[(i * 2) + 1]) {
                ...
                if (!cue.isNormalCue()) {
                    ...
                    normalCueTextBuilder.append(firstNormalCue.text).append(IOUtils.LINE_SEPARATOR_UNIX).append(cue.text);
                } else {
                    normalCueTextBuilder.append(IOUtils.LINE_SEPARATOR_UNIX).append(cue.text);
                }
            }
            i++;
        }
