...
public final class BlockingObservable<T> {
    ...
    private final Observable<? extends T> f765o;

    /* renamed from: rx.observables.BlockingObservable.1 */
    class C15781 extends Subscriber<T> {
        final /* synthetic */ AtomicReference val$exceptionFromOnError;
        final /* synthetic */ CountDownLatch val$latch;
        final /* synthetic */ Action1 val$onNext;

        C15781(CountDownLatch countDownLatch, AtomicReference atomicReference, Action1 action1) {
    ...
    class C15792 implements Iterable<T> {
        C15792() {
        }
    ...
    class C15803 extends Subscriber<T> {
        final /* synthetic */ CountDownLatch val$latch;
        final /* synthetic */ AtomicReference val$returnException;
        final /* synthetic */ AtomicReference val$returnItem;

        C15803(CountDownLatch countDownLatch, AtomicReference atomicReference, AtomicReference atomicReference2) {
            this.val$latch = countDownLatch;
    ...
    class C15814 extends Subscriber<T> {
        final /* synthetic */ CountDownLatch val$cdl;
        final /* synthetic */ Throwable[] val$error;

        C15814(Throwable[] thArr, CountDownLatch countDownLatch) {
    ...
    class C15825 extends Subscriber<T> {
        final /* synthetic */ NotificationLite val$nl;
        final /* synthetic */ BlockingQueue val$queue;

        C15825(BlockingQueue blockingQueue, NotificationLite notificationLite) {
    ...
    class C15836 extends Subscriber<T> {
        final /* synthetic */ NotificationLite val$nl;
        final /* synthetic */ BlockingQueue val$queue;
        final /* synthetic */ Producer[] val$theProducer;

        C15836(BlockingQueue blockingQueue, NotificationLite notificationLite, Producer[] producerArr) {
            this.val$queue = blockingQueue;
            this.val$nl = notificationLite;
    ...
    class C15847 implements Action0 {
        final /* synthetic */ BlockingQueue val$queue;

        C15847(BlockingQueue blockingQueue) {
            this.val$queue = blockingQueue;
        }
    ...
    class C15858 implements Action1<Throwable> {
        C15858() {
        }
    ...
    class C15869 implements Observer<T> {
        final /* synthetic */ Action0 val$onCompleted;
        final /* synthetic */ Action1 val$onError;
        final /* synthetic */ Action1 val$onNext;

        C15869(Action1 action1, Action1 action12, Action0 action0) {
            this.val$onNext = action1;
    ...
    public void forEach(Action1<? super T> onNext) {
        ...
        BlockingUtils.awaitForComplete(latch, this.f765o.subscribe(new C15781(latch, exceptionFromOnError, onNext)));
    ...
    public Iterator<T> getIterator() {
        return BlockingOperatorToIterator.toIterator(this.f765o);
    }

    public T first() {
        return blockForSingle(this.f765o.first());
    }

    public T first(Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f765o.first(predicate));
    }

    public T firstOrDefault(T defaultValue) {
        return blockForSingle(this.f765o.map(UtilityFunctions.identity()).firstOrDefault(defaultValue));
    }

    public T firstOrDefault(T defaultValue, Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f765o.filter(predicate).map(UtilityFunctions.identity()).firstOrDefault(defaultValue));
    }

    public T last() {
        return blockForSingle(this.f765o.last());
    }

    public T last(Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f765o.last(predicate));
    }

    public T lastOrDefault(T defaultValue) {
        return blockForSingle(this.f765o.map(UtilityFunctions.identity()).lastOrDefault(defaultValue));
    }

    public T lastOrDefault(T defaultValue, Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f765o.filter(predicate).map(UtilityFunctions.identity()).lastOrDefault(defaultValue));
    }

    public Iterable<T> mostRecent(T initialValue) {
        return BlockingOperatorMostRecent.mostRecent(this.f765o, initialValue);
    }

    public Iterable<T> next() {
        return BlockingOperatorNext.next(this.f765o);
    }

    public Iterable<T> latest() {
        return BlockingOperatorLatest.latest(this.f765o);
    }

    public T single() {
        return blockForSingle(this.f765o.single());
    }

    public T single(Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f765o.single(predicate));
    }

    public T singleOrDefault(T defaultValue) {
        return blockForSingle(this.f765o.map(UtilityFunctions.identity()).singleOrDefault(defaultValue));
    }

    public T singleOrDefault(T defaultValue, Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f765o.filter(predicate).map(UtilityFunctions.identity()).singleOrDefault(defaultValue));
    }

    public Future<T> toFuture() {
        return BlockingOperatorToFuture.toFuture(this.f765o);
    }
    ...
    private T blockForSingle(Observable<? extends T> observable) {
        ...
        CountDownLatch latch = new CountDownLatch(1);
        BlockingUtils.awaitForComplete(latch, observable.subscribe(new C15803(latch, returnException, returnItem)));
    ...
    public void subscribe() {
        ...
        BlockingUtils.awaitForComplete(cdl, this.f765o.subscribe(new C15814(error, cdl)));
        Throwable e = error[0];
        if (e == null) {
            return;
        }
    ...
    public void subscribe(Observer<? super T> observer) {
        ...
        Subscription s = this.f765o.subscribe(new C15825(queue, nl));
        while (true) {
            try {
                Object o = queue.poll();
                if (o == null) {
    ...
    public void subscribe(Action1<? super T> onNext) {
        subscribe(onNext, new C15858(), Actions.empty());
    }
    ...
    public void subscribe(Action1<? super T> onNext, Action1<? super Throwable> onError, Action0 onCompleted) {
        subscribe(new C15869(onNext, onError, onCompleted));
    }
}
