...
public class GooglePushServices implements UpsightGooglePushServicesApi {
    ...
    class C11051 implements OnSubscribe<String> {
        ...
        public void call(Subscriber<? super String> subscriber) {
            try {
                String registrationId = GoogleCloudMessaging.getInstance(GooglePushServices.this.mUpsight).register(this.val$projectId);
    ...
    class C11062 implements Observer<String> {
        C11062() {
        }
    ...
    class C11073 implements OnSubscribe<String> {
        C11073() {
        }
    ...
    class C11084 implements Observer<String> {
        C11084() {
        }
    ...
    class C11095 implements Action1<Config> {
        ...
        public void call(Config config) {
            ...
            if (!this.val$pushToken.equals(GooglePushServices.this.getRegistrationId()) || currentTime - GooglePushServices.this.getLastPushTokenRegistrationTime() > config.pushTokenTtl) {
                UpsightCommRegisterEvent.createBuilder().setToken(this.val$pushToken).record(GooglePushServices.this.mUpsight);
                GooglePushServices.this.storeRegistrationInfo(this.val$pushToken, currentTime);
            }
        }
    }
    ...
    public synchronized void register(OnRegisterListener listener) {
        if (listener == null) {
            ...
            if (!this.mUpsight.getPackageName().equals(gcmAuthority) || TextUtils.isEmpty(gcmSenderId)) {
                this.mLogger.m576e(LOG_TAG, "Registration aborted, wrong or no value for com.upsight.gcm was defined", new Object[0]);
                if (!this.mUpsight.getPackageName().equals(gcmAuthority)) {
                    this.mLogger.m576e(LOG_TAG, "Check that the package name of your application is specified correctly", new Object[0]);
                }
                if (TextUtils.isEmpty(gcmSenderId)) {
                    this.mLogger.m576e(LOG_TAG, "Check that your GCM sender id is specified correctly", new Object[0]);
                }
    ...
    private void registerInBackground(String projectId) {
        this.mRegistrationIsInProgress = true;
        Observable.create(new C11051(projectId)).subscribeOn(this.mComputationScheduler).observeOn(HandlerScheduler.from(this.mUiThreadHandler)).subscribe(new C11062());
    }
    ...
    private void unregisterInBackground() {
        ...
        Observable.create(new C11073()).subscribeOn(this.mComputationScheduler).observeOn(HandlerScheduler.from(this.mUiThreadHandler)).subscribe(new C11084());
    }

    private void registerPushToken(String pushToken) {
        try {
            this.mPushConfigManager.fetchCurrentConfigObservable().subscribeOn(this.mUpsight.getCoreComponent().subscribeOnScheduler()).observeOn(this.mUpsight.getCoreComponent().observeOnScheduler()).subscribe(new C11095(pushToken));
        } catch (IOException e) {
            this.mLogger.m576e(LOG_TAG, "Failed to fetch push configurations", e);
        }
    }
    ...
    private boolean hasPlayServices() {
        ...
        this.mLogger.m576e(LOG_TAG, "Google play service is not available: ", GooglePlayServicesUtil.getErrorString(GooglePlayServicesUtil.isGooglePlayServicesAvailable(this.mUpsight)));
        return false;
    }
