...
public class QueueBuilder {
    ...
    public Queue build(String name, QueueConfig config, Selector<Schema> schemaSelectorByName, Selector<Schema> schemaSelectorByType) {
        UpsightEndpoint upsightEndpoint = new UpsightEndpoint(prepareEndpoint(config.getEndpointAddress()), this.mSignatureVerifier, this.mGson, this.mJsonParser, this.mResponseLoggingGson, this.mLogger);
        BatchSender sender = new BatchSender(this.mUpsight, config.getBatchSenderConfig(), this.mRetryExecutor, this.mSendExecutor, endpoint, (ResponseParser) this.mResponseParserProvider.get(), this.mJsonParser, this.mClock, this.mLogger);
    ...
    private String getAppVersionName(Context context) {
        ...
        try {
            ...
            this.mLogger.m576e(QueueBuilder.class.getSimpleName(), "Could not get package info", e);
        } catch (UnsupportedEncodingException e2) {
            this.mLogger.m576e(QueueBuilder.class.getSimpleName(), "UTF-8 encoding not supported", e2);
        }
        return versionName;
    }
}
