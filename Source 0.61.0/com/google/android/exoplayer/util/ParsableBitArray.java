...
public final class ParsableBitArray {
    ...
    public int readBits(int numBits) {
        ...
        for (int i = 0; i < wholeBytes; i++) {
            int byteValue;
            if (this.bitOffset != 0) {
                byteValue = ((this.data[this.byteOffset] & NalUnitUtil.EXTENDED_SAR) << this.bitOffset) | ((this.data[this.byteOffset + 1] & NalUnitUtil.EXTENDED_SAR) >>> (8 - this.bitOffset));
            } else {
                byteValue = this.data[this.byteOffset];
            }
            numBits -= 8;
            returnValue |= (byteValue & NalUnitUtil.EXTENDED_SAR) << numBits;
            this.byteOffset++;
        }
        if (numBits > 0) {
            int nextBit = this.bitOffset + numBits;
            byte writeMask = (byte) (NalUnitUtil.EXTENDED_SAR >> (8 - numBits));
            if (nextBit > 8) {
                returnValue |= (((this.data[this.byteOffset] & NalUnitUtil.EXTENDED_SAR) << (nextBit - 8)) | ((this.data[this.byteOffset + 1] & NalUnitUtil.EXTENDED_SAR) >> (16 - nextBit))) & writeMask;
                this.byteOffset++;
            } else {
                returnValue |= ((this.data[this.byteOffset] & NalUnitUtil.EXTENDED_SAR) >> (8 - nextBit)) & writeMask;
                if (nextBit == 8) {
                    this.byteOffset++;
                }
            }
