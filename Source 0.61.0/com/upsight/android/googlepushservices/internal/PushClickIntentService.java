        ...
        if (Upsight.isEnabled(getApplicationContext())) {
            ...
            if (extension != null) {
                ...
                if (State.BACKGROUND.name().equals(((ApplicationStatus) upsight.getDataStore().fetchObservable(ApplicationStatus.class).toBlocking().first()).getState().name())) {
                    sessionManager.startSession(PushSessionInitializer.fromExtras(extras));
                }
                ...
                if (extras.containsKey(PARAM_KEY_PUSH_CONTENT_ID)) {
                    ...
                    try {
                        ...
                        upsight.getLogger().m577e(LOG_TAG, e, "Could not construct \"content_provider\" bundle in \"upsight.content.unrendered\"", new Object[0]);
                    }
                }
