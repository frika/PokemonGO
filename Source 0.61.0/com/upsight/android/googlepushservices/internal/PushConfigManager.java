...
import com.upsight.android.googlepushservices.C1099R;
...
public class PushConfigManager {
    ...
    class C11101 implements Func1<Config, Boolean> {
        C11101() {
        }
    ...
    class C11112 implements Func1<UpsightConfiguration, Config> {
        C11112() {
        }
    ...
    class C11123 implements Func1<UpsightConfiguration, Boolean> {
        C11123() {
        }
    ...
    public Observable<Config> fetchCurrentConfigObservable() throws IOException {
        return this.mDataStore.fetchObservable(UpsightConfiguration.class).filter(new C11123()).map(new C11112()).filter(new C11101()).firstOrDefault(parseConfiguration(IOUtils.toString(this.mUpsight.getResources().openRawResource(C1099R.raw.push_config))));
    }
    ...
    private Config parseConfiguration(String jsonConfig) {
        ...
        try {
            ...
            this.mLogger.m576e(LOG_TAG, "Could not parse incoming config", e);
            return config;
        }
    }
}
