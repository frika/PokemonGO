...
public interface HttpDataSource extends UriDataSource {
    ...
    static class C03191 implements Predicate<String> {
        C03191() {
        }

        public boolean evaluate(String contentType) {
            contentType = Util.toLowerInvariant(contentType);
            return (TextUtils.isEmpty(contentType) || ((contentType.contains(PendingDialog.TEXT) && !contentType.contains(MimeTypes.TEXT_VTT)) || contentType.contains(Keys.HTML) || contentType.contains("xml"))) ? false : true;
        }
    }
    ...
    static {
        REJECT_PAYWALL_TYPES = new C03191();
    }
}
